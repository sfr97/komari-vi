...
# Listen on all interfaces (LAN/WAN) - recommended to enable REALM_API_KEY
export REALM_API_KEY="your-api-key"
realm api --bind 0.0.0.0 --port 8080
...
Note: the API server binds to `127.0.0.1` by default (localhost only). Use `--bind 0.0.0.0` to expose it externally, and enable `REALM_API_KEY` for security.
...
| `GET` | `/instances/{id}/stats` | Get instance traffic & connection statistics |
| `GET` | `/instances/{id}/route` | Get instance active route / preferred backend (failover) |
| `GET` | `/instances/{id}/connections` | Get instance connection list (paged) |
...
# Get instance traffic & connection statistics (TCP+UDP, since API server start)
# - total_inbound_bytes: client -> remote
# - total_outbound_bytes: remote -> client
curl -H "X-API-Key: your-api-key" \
     http://localhost:8080/instances/{instance-id}/stats

# Get instance connection list (paged)
curl -H "X-API-Key: your-api-key" \
     "http://localhost:8080/instances/{instance-id}/connections?protocol=tcp&limit=100&offset=0"

# For UDP, `protocol=udp` returns UDP sessions (not TCP-style connections)
curl -H "X-API-Key: your-api-key" \
     "http://localhost:8080/instances/{instance-id}/connections?protocol=udp&limit=100&offset=0"

# If `protocol` is omitted, it returns both TCP connections and UDP sessions
curl -H "X-API-Key: your-api-key" \
     "http://localhost:8080/instances/{instance-id}/connections?limit=100&offset=0"
...
Example response for `/instances/{id}/stats`:
```json
{
  "id": "uuid-1",
  "total_inbound_bytes": 123456,
  "total_outbound_bytes": 654321,
  "total_connections": 42,
  "current_connections": 2,
  "tcp_inbound_bytes": 120000,
  "tcp_outbound_bytes": 640000,
  "tcp_total_connections": 40,
  "tcp_current_connections": 1,
  "udp_inbound_bytes": 3456,
  "udp_outbound_bytes": 14321,
  "udp_total_sessions": 2,
  "udp_current_sessions": 1,
  "connections_by_backend": { "1.1.1.1:443": 2 },
  "bytes_by_backend": { "1.1.1.1:443": { "inbound_bytes": 123456, "outbound_bytes": 654321 } }
}
```

Notes:
- For UDP, the API reports **sessions** (keyed by `src_ip:src_port` association and removed after `associate_timeout` idle timeout).
- Deprecated fields kept for compatibility: `udp_total_connections`, `udp_current_connections` (same values as `udp_total_sessions`, `udp_current_sessions`).
- `connections_by_backend` aggregates **current** TCP connections plus **current** UDP sessions by backend address.
- `bytes_by_backend` aggregates **cumulative** bytes for TCP by selected backend; UDP bytes are attributed to `remote` (the primary backend) because per-session UDP backend selection is not tracked.

Example response for `/instances/{id}/route`:
```json
{
  "id": "uuid-1",
  "strategy": "failover",
  "preferred_backend": "2.2.2.2:443",
  "last_success_backend": "2.2.2.2:443",
  "backends": [
    { "addr": "1.1.1.1:443", "role": "primary", "state": "backoff", "backoff_until_ms": 123, "ok_recent": false },
    { "addr": "2.2.2.2:443", "role": "backup", "state": "healthy", "backoff_until_ms": null, "ok_recent": true }
  ],
  "connections_by_backend": { "1.1.1.1:443": 1, "2.2.2.2:443": 1 },
  "bytes_by_backend": {
    "1.1.1.1:443": { "inbound_bytes": 100, "outbound_bytes": 200 },
    "2.2.2.2:443": { "inbound_bytes": 300, "outbound_bytes": 400 }
  },
  "updated_at": "2025-12-26T00:00:00Z"
}
```

Example response for `/instances/{id}/connections`:
```json
{
  "id": "uuid-1",
  "protocol": "tcp",
  ...
  "offset": 0,
  "connections": [
    { "src_ip": "192.168.1.10", "src_port": 52344, "duration_secs": 12, "backend": "1.1.1.1:443" },
    { "src_ip": "192.168.1.11", "src_port": 52345, "duration_secs": 3, "backend": "2.2.2.2:443" }
  ]
}
```

Example response for `/instances/{id}/connections?protocol=udp`:
```json
{
  "id": "uuid-1",
  "protocol": "udp",
  ...
  "offset": 0,
  "sessions": [
    { "src_ip": "192.168.1.10", "src_port": 52344, "duration_secs": 12, "backend": "8.8.8.8:53" }
  ]
}
```

Example response for `/instances/{id}/connections` (no `protocol`, returns both):
```json
{
  "id": "uuid-1",
  "protocol": "all",
...
When a request fails, the API returns a JSON body in the following format:
...
#### POST /instances
- `201` - Instance created successfully (check the returned `status` field; it may be `Failed(...)` if startup fails)
- `200` - Instance updated successfully (upsert; `id` already existed)
- `400` - Invalid configuration or invalid `id`
- `401` - Unauthorized access
- `500` - Internal server error
...
- `200` - Instance statistics returned
- `401` - Unauthorized access
- `404` - Instance not found
- `500` - Internal server error

#### GET /instances/{id}/route
- `200` - Instance route returned
- `401` - Unauthorized access
- `404` - Instance not found
- `500` - Internal server error

#### GET /instances/{id}/connections
- `200` - Instance connection list returned
- `400` - Invalid query
- `401` - Unauthorized access
- `404` - Instance not found
- `500` - Internal server error
...
#### PUT /instances/{id}
- `200` - Instance updated successfully (check the returned `status` field; it may be `Failed(...)` if restart fails)
...
- `200` - Instance started successfully (check the returned `status` field; it may be `Failed(...)` if startup fails)
- `400` - Invalid configuration
- `401` - Unauthorized access
- `404` - Instance not found
...
- `200` - Instance restarted successfully (check the returned `status` field; it may be `Failed(...)` if restart fails)
- `400` - Invalid configuration
- `401` - Unauthorized access
- `404` - Instance not found
- `500` - Internal server error
