...
For `failover`, you can omit weights (use `failover` or `failover:`).

Failover behavior:

- On each new TCP connection, it prefers `remote` (primary).
- If `remote` is unavailable, it fails over to `extra_remotes` in order.
- Once `remote` becomes available again, new connections will switch back to `remote`.

To reduce connection latency during outages, failover maintains a shared health state:

- A background probe loop (enabled by default) periodically checks peers.
- Connection selection prioritizes peers that were recently healthy, and skips peers that are in backoff.
- If the health state is stale or inaccurate, it falls back to a short fail-fast connect timeout before trying the next peer.
- If the client disconnects before a remote connection is established, the connection attempt stops (no endless retries).
- To prevent pathological configs, failover options are sanitized (e.g. retry window is capped).
...
Failover example (recommended):

```toml
[[endpoints]]
remote = "1.1.1.1:443"
extra_remotes = ["2.2.2.2:443", "3.3.3.3:443"]
balance = "failover"

[endpoints.network]
failover_probe_interval_ms = 2000
failover_probe_timeout_ms = 200
failover_failfast_timeout_ms = 250
```
...
#### network.failover_probe_interval_ms: unsigned int

Only effective when `endpoint.balance = "failover"`.

Enable a background probe loop when > 0. It periodically checks remote peers and maintains a shared health state so new connections can pick a healthy peer immediately (without waiting for `tcp_timeout`).

default: 2000

#### network.failover_probe_timeout_ms: unsigned int

Only effective when `endpoint.balance = "failover"`.

Connect timeout for each probe attempt.

default: 200

#### network.failover_failfast_timeout_ms: unsigned int

Only effective when `endpoint.balance = "failover"`.

When a peer is not recently healthy, new connections will try it with this fail-fast timeout before failing over to the next peer.

default: 250

#### network.failover_ok_ttl_ms: unsigned int

Only effective when `endpoint.balance = "failover"`.

Treat a peer as healthy if it had a successful connection within this TTL.

default: 6000

#### network.failover_backoff_base_ms: unsigned int

Only effective when `endpoint.balance = "failover"`.

Base backoff after a failure (exponential).

default: 500

#### network.failover_backoff_max_ms: unsigned int
...
#### network.failover_retry_window_ms: unsigned int

Only effective when `endpoint.balance = "failover"`.

When > 0, if all peers fail to connect, Realm will keep retrying within this window before giving up.

default: 0 (disabled)

#### network.failover_retry_sleep_ms: unsigned int

Only effective when `endpoint.balance = "failover"`.

Sleep duration between retry rounds.
