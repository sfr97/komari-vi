...
pub struct NetConf {
    ...
    // failover (balance=failover) options, milliseconds
    #[serde(default)]
    #[serde(skip_serializing_if = "Option::is_none")]
    ...
    #[serde(default)]
    #[serde(skip_serializing_if = "Option::is_none")]
    pub failover_probe_timeout_ms: Option<u64>,

    #[serde(default)]
    #[serde(skip_serializing_if = "Option::is_none")]
    pub failover_failfast_timeout_ms: Option<u64>,

    #[serde(default)]
    #[serde(skip_serializing_if = "Option::is_none")]
    pub failover_ok_ttl_ms: Option<u64>,

    #[serde(default)]
    #[serde(skip_serializing_if = "Option::is_none")]
    pub failover_backoff_base_ms: Option<u64>,

    #[serde(default)]
    #[serde(skip_serializing_if = "Option::is_none")]
    pub failover_backoff_max_ms: Option<u64>,
    ...
    #[serde(default)]
    #[serde(skip_serializing_if = "Option::is_none")]
    pub failover_retry_sleep_ms: Option<u64>,
}
...
impl Config for NetConf {
    ...
    fn is_empty(&self) -> bool {
        crate::empty![self =>
            ...
            failover_probe_interval_ms, failover_probe_timeout_ms,
            failover_failfast_timeout_ms, failover_ok_ttl_ms,
            failover_backoff_base_ms, failover_backoff_max_ms,
            failover_retry_window_ms, failover_retry_sleep_ms
        ]
    }
    ...
    fn build(self) -> Self::Output {
        ...
        let conn_opts = ConnectOpts {
            ...
            #[cfg(feature = "balance")]
            failover: {
                use realm_core::endpoint::FailoverOpts;
                let mut fo = FailoverOpts::default();
                if let Some(v) = self.failover_probe_interval_ms {
                    fo.probe_interval_ms = v;
                }
                if let Some(v) = self.failover_probe_timeout_ms {
                    fo.probe_timeout_ms = v;
                }
                if let Some(v) = self.failover_failfast_timeout_ms {
                    fo.failfast_timeout_ms = v;
                }
                if let Some(v) = self.failover_ok_ttl_ms {
                    fo.ok_ttl_ms = v;
                }
                if let Some(v) = self.failover_backoff_base_ms {
                    fo.backoff_base_ms = v;
                }
                if let Some(v) = self.failover_backoff_max_ms {
                    fo.backoff_max_ms = v;
                }
                if let Some(v) = self.failover_retry_window_ms {
                    fo.retry_window_ms = v;
                }
                if let Some(v) = self.failover_retry_sleep_ms {
                    fo.retry_sleep_ms = v;
                }
                fo.sanitize();
                fo
    ...
    fn rst_field(&mut self, other: &Self) -> &mut Self {
        ...
        rst!(self, failover_probe_interval_ms, other);
        rst!(self, failover_probe_timeout_ms, other);
        rst!(self, failover_failfast_timeout_ms, other);
        rst!(self, failover_ok_ttl_ms, other);
        rst!(self, failover_backoff_base_ms, other);
        rst!(self, failover_backoff_max_ms, other);
        rst!(self, failover_retry_window_ms, other);
        rst!(self, failover_retry_sleep_ms, other);
        self
    }
    ...
    fn take_field(&mut self, other: &Self) -> &mut Self {
        ...
        take!(self, failover_probe_interval_ms, other);
        take!(self, failover_probe_timeout_ms, other);
        take!(self, failover_failfast_timeout_ms, other);
        take!(self, failover_ok_ttl_ms, other);
        take!(self, failover_backoff_base_ms, other);
        take!(self, failover_backoff_max_ms, other);
        take!(self, failover_retry_window_ms, other);
        take!(self, failover_retry_sleep_ms, other);
        self
    }
    ...
    fn from_cmd_args(matches: &clap::ArgMatches) -> Self {
        ...
        Self {
            ...
            send_proxy_version,
            accept_proxy_timeout,
            failover_probe_interval_ms: None,
            failover_probe_timeout_ms: None,
            failover_failfast_timeout_ms: None,
            failover_ok_ttl_ms: None,
            failover_backoff_base_ms: None,
            failover_backoff_max_ms: None,
            failover_retry_window_ms: None,
            failover_retry_sleep_ms: None,
        }
    }
}
